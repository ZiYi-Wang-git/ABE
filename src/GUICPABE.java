...
public class GUICPABE {
    ...
    private static void createWindow() {
        JFrame frame = new JFrame("智能电网用电信息加解密系统V1.0");
...
//        tabbedPane.addTab("帮助", new ImageIcon(GUICPABE.class.getResource("images/help.png")), helpPanel,"Tab 1 tooltip");
    ...
    private static JPanel helpUI(final JFrame frame){
        ...
        textArea.append("本软件可实现安全、灵活、高效的数据共享。数据基于一个由多个属性组成的树形访问策略进行加密，解密私钥基于一组属性集合生成。在私钥的属性集合满足访问策略的情况下，可以成功解密密文。\n");
        textArea.append("一、软件包含4部分：系统初始化，生成私钥，加密文件，解密文件。\n" +
                        "1.各项功能可以独立执行;\n" +
                        "2.每项中的红色标星部分为必须进行设置。\n");
        textArea.append("二、系统初始化\n" +
                        "1.用于生成系统公钥和主秘钥；\n" +
                        "2.系统公钥公开发布；\n" +
                        "3.系统主秘钥必须由管理者安全妥善保管；\n" +
                        "4.可以为不同的系统生成不同的公钥和主秘钥对，但二者必须配对使用。\n");
        textArea.append("三、生成私钥\n" +
        ...
        textArea.append("注意：以上过程中，在一个系统内必须保证所使用的系统公钥、主秘钥保持一致。\n");
...
//        cmb.addItem("--请选择--");    //向下拉列表中添加一项
        ...
        setupButton.addActionListener(e -> {
            ...
            //获取选择的安全等级
            pairingParametersFileName =  "resources/security_params/security_level_" + cmb.getSelectedItem() + "bits.properties";
    ...
    private static JPanel EncryptUI(final JFrame frame){
        ...
        dataButton.addActionListener(e -> {   //与js语法糖不同  为lambda
            JFileChooser fileChooser = new JFileChooser(lastPath);
